      ...
      <main className="bg-gray-100 font-montserrat">
        <header className="h-24 sm:h-32 flex items-center">
          <div className="container mx-auto px-6 sm:px-12 flex items-center justify-between">
            <div
              style={{ color: "#0d6dfdd8" }}
              className="font-black text-blue-900 text-2xl flex items-start"
            >
              {testerInfo.code.user.company}{" "}
              <span className="w-3 h-3 rounded-full bg-blue-600 ml-2"> </span>
            </div>
          </div>
        </header>
        <div className="container mx-auto px-6 sm:px-12 flex flex-col-reverse sm:flex-row relative">
          ...
          <div className="sm:w-5/12 xl:w-4/12 flex flex-col items-start sm:items-end sm:text-right ml-auto mt-8 sm:mt-0 relative z-10 xl:pt-20 mb-16 sm:mb-0">
            <h1
              style={{ color: "#0d6dfdd8" }}
              className="text-4xl lg:text-5xl text-blue-900 leading-none mb-4 font-black"
            >
              Bienvenido {testerInfo.email}
            </h1>
            <p
              style={{ color: "#035ce0ad" }}
              className="lg:text-lg mb-4 sm:mb-12 text-blue-900"
            >
              Para seguir con el proceso de aplicación la empresa{" "}
              <strong>
                <u>{testerInfo.code.user.company}</u>
              </strong>{" "}
              solicita que contestes el siguiente conjunto de examenes que fue
              armado para la vacante de <u>{testerInfo.code.namePosition}</u>.
            </p>
            {testerInfo.answerTest ? (
              <p
                style={{ color: "#035ce0ad" }}
                className="lg:text-lg mb-4 sm:mb-12 text-blue-900"
              >
                El examen ya fue contestado, se pondrán en contacto para el
                seguimiento de la vacante.
              </p>
            ) : (
              <Link
                to={`/tester/${testerInfo._id}/${testerInfo.code._id}`}
                className="font-semibold text-lg bg-blue-600 hover:bg-blue-400 text-white py-3 px-12 rounded-full"
              >
                Empezar Prueba
              </Link>
            )}
